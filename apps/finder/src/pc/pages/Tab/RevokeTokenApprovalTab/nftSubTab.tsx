...
import { useApprovedNfts } from '../../../../api/my'
...
import RevokeDialog, { NftProps } from './revokeDialog'
...
const NftSubTab = () => {
    ...
    const { data, refresh } = useApprovedNfts(selectedAddress, page, 20)
    const { results, currentPage, totalPage, totalCount } = data

    const [dialogState, setDialogState] = useState<NftProps>({})
    ...
    return (
        <>
            <RevokeDialog type="nft" refresh={refresh} open={isShow} onClose={off} {...dialogState} />
            {totalPage === 0 ? (
                <Empty />
            ) : (
                <>
                    <Table columnSizeList={[312, 280, 120, 200, 276]} totalCount={totalCount}>
                        <TableHeaders>
