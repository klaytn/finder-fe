...
import { useAccountInternalTransactions } from '../../../../api/account'
...
type AccountInternalTransactionsTabProps = {
    address: string
}

const AccountInternalTransactionsTab = ({ address }: AccountInternalTransactionsTabProps) => {
    const query = useQuery()
    const page = query.get('page') || '1'
    const tabId = query.get('tabId')

    const { currentPage, totalPage, results, totalCount } = useAccountInternalTransactions(address, page)
    const {
        paging: { limit },
    } = useServerConfig()
...
export default AccountInternalTransactionsTab
