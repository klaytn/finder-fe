...
import { useAccountTokenTransfers } from '../../../../api/account'
...
type AccountTokenTransfersTabProps = {
    address: string
}

const AccountTokenTransfersTab = ({ address }: AccountTokenTransfersTabProps) => {
    const query = useQuery()
    const page = query.get('page') || '1'
    const tabId = query.get('tabId')

    const { results, currentPage, totalPage, totalCount } = useAccountTokenTransfers(address, page)
    const {
        paging: { limit },
    } = useServerConfig()
    ...
    return (
        <Flex>
            ...
            {results.map((tokenTransfer, index) => (
                <TransactionItemBox key={index} data={tokenTransfer} />
            ))}
...
export default AccountTokenTransfersTab
