...
import { useTokenTransfers } from '../../../../api/transaction'
...
type TransactionTokenTransfersTabProps = {
    hash: string
}

const TransactionTokenTransfersTab = ({ hash }: TransactionTokenTransfersTabProps) => {
    const query = useQuery()
    const page = query.get('page') || '1'
    const tabId = query.get('tabId')

    const { results, currentPage, totalPage, totalCount } = useTokenTransfers(hash, page)
    const {
        paging: { limit },
    } = useServerConfig()
