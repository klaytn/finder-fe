...
import { useEventLogs } from '../../../../api/transaction'
...
type TransactionEventLogsTabProps = {
    hash: string
}

const TransactionEventLogsTab = ({ hash }: TransactionEventLogsTabProps) => {
    const query = useQuery()
    const page = query.get('page') || '1'
    const tabId = query.get('tabId')

    const { currentPage, totalPage, totalCount, results } = useEventLogs(hash, page)
    const {
        paging: { limit },
    } = useServerConfig()
...
export default TransactionEventLogsTab
